...
export default function ChatPlayground() {
  ...
  const modelSelectorNode = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border/80 bg-card/80 hover:bg-card text-xs font-mono text-foreground transition-all cursor-pointer shadow-sm outline-none">
      ...
      <DropdownMenuContent align="end" className="w-60 p-1.5 space-y-1 dark bg-surface-mid border border-cream-soft/60 text-cream shadow-2xl backdrop-blur-xl rounded-xl z-50">
        {[
  ...
  return (
    <AppShell headerActions={modelSelectorNode}>

      {/* Main Grid: Clean Chat Stream + Compact Security Telemetry */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 h-[calc(100vh-190px)] min-h-[500px]">
        {/* Left Column: Chat Window */}
        <div className="lg:col-span-8 flex flex-col rounded-xl border border-border/70 bg-card/40 backdrop-blur-md overflow-hidden shadow-lg">
          {/* Message History */}
          <div className="flex-1 p-5 overflow-y-auto space-y-4 text-sm font-sans">
            {messages.map((msg) => (
              ...
              >
                ...
                <div className="space-y-1">
                  <div
                    className={`rounded-xl px-4 py-3 leading-relaxed shadow-sm ${
                      msg.sender === "user"
            ...
            {isLoading && (
              <div className="flex gap-3 mr-auto">
                <div className="h-7 w-7 rounded-lg bg-emerald-950/80 border border-emerald-500/30 text-emerald-400 flex items-center justify-center animate-spin">
                  <RefreshCw className="h-3.5 w-3.5" />
                </div>
                <div className="rounded-xl px-4 py-3 bg-muted/40 border border-border/60 text-muted-foreground text-xs flex items-center gap-2 font-mono">
          ...
          {/* Input Area */}
          <div className="p-3 border-t border-border bg-card/80 flex gap-2 items-center">
            <input
              type="text"
              value={inputPrompt}
              onChange={(e) => setInputPrompt(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleSend()}
              placeholder="Type a prompt to test PII masking or prompt injection defense..."
              className="flex-1 bg-background/80 border border-border/60 rounded-lg px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/60 font-sans"
            />
            <Button
              onClick={() => handleSend()}
              disabled={isLoading || !inputPrompt.trim()}
              className="btn-primary h-[40px] px-4 flex items-center gap-2 cursor-pointer"
            >
        ...
        {/* Right Column: Compact Security Inspector Card */}
